...
The other four hold up, with one repair. Ratio's *values* were always fine —
4.8 is in the useful part of the curve and above 8:1 buys almost nothing — but
the pot was `LINEAR(1 20)` while the compressor uses `1 − 1/ratio`, which
saturates. The first twentieth of the travel covered more than half the effect
and the last 63% covered 0.7 dB. It is `EXPONENTIAL(1 20)` now: the same
values, the same reach, spread across the knob instead of piled at one end.

Attack costs 5 dB of THD across its whole range while changing the squeeze by
more than twice that. Release is right for guitar and marginal an octave below.
