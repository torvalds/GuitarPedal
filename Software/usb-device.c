...
static tusb_desc_device_t const desc_device =
{
    ...
    .idVendor           = 0xFFFF,
    .idProduct          = 0x0003, // Composite Device
    .bcdDevice          = 0x0100,
...
static char const* string_desc_arr[] = {
    ...
    "Linus",                        // 1: Manufacturer
    "Linus Pedal",                  // 2: Product
    "0",                            // 3: Serial
    "UAC2 Audio",                   // 4: Audio Interface
...
// Static control responses to ensure memory persists during DMA transfer
static audio20_control_range_2_n_t(1) vol_range_resp;
static audio20_control_range_4_n_t(1) sample_freq_rng_resp;
static audio20_desc_channel_cluster_t conn_cluster_resp;
...
bool tud_audio_get_req_entity_cb(uint8_t rhport, tusb_control_request_t const * p_request)
{
    ...
    if (entityID == UAC2_ENTITY_MIC_INPUT_TERMINAL || entityID == UAC2_ENTITY_SPK_INPUT_TERMINAL) {
        if (ctrlSel == AUDIO20_TE_CTRL_CONNECTOR) {
            conn_cluster_resp.bNrChannels = 2;
            conn_cluster_resp.bmChannelConfig = (audio20_channel_config_t) 0;
            conn_cluster_resp.iChannelNames = 0;
            return tud_audio_buffer_and_schedule_control_xfer(rhport, p_request, (void *) &conn_cluster_resp, sizeof(conn_cluster_resp));
        }
    } else if (entityID == UAC2_ENTITY_MIC_FEATURE_UNIT || entityID == UAC2_ENTITY_SPK_FEATURE_UNIT) {
        if (ctrlSel == AUDIO20_FU_CTRL_MUTE) {
            ...
            if (p_request->bRequest == AUDIO20_CS_REQ_CUR) {
                ...
                vol_range_resp.wNumSubRanges = 1;
                vol_range_resp.subrange[0].bMin = -90 * 256; // -90 dB
                vol_range_resp.subrange[0].bMax = 90 * 256;  // +90 dB
                vol_range_resp.subrange[0].bRes = 1 * 256;   // 1 dB step
                return tud_audio_buffer_and_schedule_control_xfer(rhport, p_request, (void *) &vol_range_resp, sizeof(vol_range_resp));
            }
        }
    } else if (entityID == UAC2_ENTITY_CLOCK) {
        if (ctrlSel == AUDIO20_CS_CTRL_SAM_FREQ) {
            if (p_request->bRequest == AUDIO20_CS_REQ_CUR) {
                ...
                sample_freq_rng_resp.wNumSubRanges = 1;
                sample_freq_rng_resp.subrange[0].bMin = 48000;
                sample_freq_rng_resp.subrange[0].bMax = 48000;
                sample_freq_rng_resp.subrange[0].bRes = 0;
                return tud_audio_buffer_and_schedule_control_xfer(rhport, p_request, (void *) &sample_freq_rng_resp, sizeof(sample_freq_rng_resp));
            }
...
// Thread-safe ring buffer via TinyUSB's tu_fifo_t
#define USB_RX_BUF_SIZE 512
static raw_sample_t usb_rx_buf_pool[USB_RX_BUF_SIZE];
TU_FIFO_DEF(usb_rx_fifo, USB_RX_BUF_SIZE, raw_sample_t, true);

void usb_audio_task(void)
{
    // Initialize FIFO on first run
    if (!tu_fifo_capacity(&usb_rx_fifo)) {
        tu_fifo_config(&usb_rx_fifo, usb_rx_buf_pool, USB_RX_BUF_SIZE, sizeof(raw_sample_t), true);
    }
    ...
    if (rx_avail) {
        ...
        // Push directly into thread-safe FIFO
        tu_fifo_write_n(&usb_rx_fifo, temp_buf, samples_read);
    }
}
...
{
    raw_sample_t sample;
    if (tu_fifo_read(&usb_rx_fifo, &sample)) {
        return (sample_t) {
