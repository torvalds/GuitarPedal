...
def generate(audio_dir, out_h, out_js, out_md):
    ...
    with open(out_md, 'w') as f:
        ...
        f.write("in.\n\n")
        f.write("- **0x0c, set one binding, in.**\n")
        f.write("  `F0 7D 0C <control> <action> <arg0> <arg1> F7`. The pedal\n")
        f.write("  replies with 0x0d whether or not it took the change: a\n")
        f.write("  refused binding is precisely the case where the two ends\n")
        f.write("  disagree, so it is the case where the answer matters.\n")
        f.write("- **0x0d, the whole table, out.** Five entries of three\n")
        f.write("  bytes, `<action> <arg0> <arg1>` each, in control order.\n")
        f.write("  Sent as part of the state dump, and again whenever the\n")
        f.write("  pedal moves a binding by itself.\n\n")
        f.write("Controls, in the order they appear in 0x0d: 0 rotary turn,\n")
        ...
        f.write("Actions: 0 none, 1 pot (`arg0` effect, `arg1` pot 0-9),\n")
        f.write("2 next pot, 3 bypass, 4 tuner, 5 scene (`arg0` scene).\n")
        ...
        f.write("defaults on every boot, and those defaults are what the\n")
        f.write("pedal did when the gestures were hardcoded.\n\n")
